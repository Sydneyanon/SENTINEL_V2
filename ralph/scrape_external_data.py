...
class ExternalDataScraper:
    ...
    async def analyze_successful_tokens(self, min_gain_percent: float = 200, max_tokens: int = 500) -> tuple[List[Dict], Dict]:
        ...
        # FALLBACK: If Moralis returns 0 tokens, use DexScreener as backup
        if len(tokens) == 0:
            logger.warning("⚠️  Moralis returned 0 tokens. Falling back to DexScreener...")
            logger.info("🔄 Fetching trending tokens from DexScreener (FREE API)...")
            dex_tokens = await self.fetch_trending_tokens_dexscreener(min_volume_24h=10000)
            # Filter for tokens with gains >= threshold
            # DexScreener returns price_change as decimal (1.33 = 133%), convert to percentage for comparison
            tokens = [t for t in dex_tokens if (t.get('price_change_24h', 0) * 100) >= min_gain_percent]
            logger.info(f"📊 DexScreener returned {len(tokens)} tokens with >={min_gain_percent}% gains")

        # For DexScreener data (when Moralis returns 0), skip MCAP filtering since price_change already filters winners
        # For Moralis data, still filter by MCAP
        if len(tokens) > 0 and 'market_cap' in tokens[0]:
            # Moralis tokens - filter by MCAP
            ...
            # DexScreener tokens - already filtered by price_change, no additional MCAP filter
            winners = tokens
            logger.info(f"✅ Found {len(winners)} tokens with >={min_gain_percent}% gains (DexScreener data)")
...
async def main():
    ...
    MIN_GAIN = 15  # 15% minimum (VERY low threshold to collect training data in dead market)

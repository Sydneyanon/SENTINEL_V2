...
class ExternalDataScraper:
    ...
    async def analyze_successful_tokens(self, min_gain_percent: float = 200, max_tokens: int = 500) -> tuple[List[Dict], Dict]:
        ...
        # FALLBACK: If pump.fun fails, use DexScreener instead
        if len(tokens) == 0:
            logger.warning("⚠️ pump.fun returned no tokens, falling back to DexScreener...")
            tokens = await self.fetch_trending_tokens_dexscreener(min_volume_24h=10000)
            logger.info(f"📊 DexScreener returned {len(tokens)} tokens as fallback")

        # For graduated tokens, filter by market cap OR price change
        # Graduated = already successful (bonded $60k+) OR gained significantly
        # Filter for tokens with mcap > $100k (moderate success) or > $500k (big winners)
        # OR tokens with >100% gain in 24h (DexScreener data)
        ...
        for token in tokens:
            ...
            # Accept if: good mcap OR strong price gain
            if mcap >= min_mcap or price_change_24h >= min_gain_percent:
                winners.append(token)

        logger.info(f"✅ Found {len(winners)} successful tokens (mcap >${min_mcap/1000}k OR >{min_gain_percent}% gain) out of {len(tokens)} total")
